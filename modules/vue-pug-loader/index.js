...
var pug_runtime = require('pug-runtime');
var Vue = require('vue/dist/vue.common.prod');
...
global.document = {
  createElement() {
    return {
      ...
      get textContent() {
        return entities.decode(this.text);
      },
    };
  },
};
...
module.exports = function(source) {
	...
	function run() {
		...
		eval('var pug = pug_runtime;'+tmplFunc.toString());
		let html = template({
			loadAssets: global.util.loadAssets,
		});
		html = Vue.compile(html);
		const tmp = `module.exports = function(vueComponent){return Object.assign(vueComponent, {render:${html.render.toString()},staticRenderFns:[${html.staticRenderFns.toString()}]})}`;
		loaderContext.callback(null, tmp);
	}
}
