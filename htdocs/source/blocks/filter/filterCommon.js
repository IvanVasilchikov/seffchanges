...
export default {
	...
	methods: {
		...
		changeTags(val) {
			this.filter.tags = [];
			const taglist = this.info.tabs.filter(el => el.name === this.activeTab)[0].tags;
			if (taglist) {
				let tags = taglist.selected;

				if (taglist.list && taglist.list.length !== 0) {
					tags = taglist.list.filter(el => el.name === val)[0];
		...
		updateFilter() {
			...
			if (this.info.tabs) {
				activeTab = this.info.tabs.filter((el) => el.name === this.activeTab)[0];
			}
			if (activeTab) {
								...
								if (field.hasOwnProperty(key)) {
									if (field.type === 'range') {
										...
										if (field.range && field.range.currency) {
											const selected = field.range.currency.values.filter((select) => select.selected)[0];
											this.$set(this.filter, `${field.range.currency.name}`, selected.value);
										}
									} else if (field.type === 'radio') {
										const checked = field.values.filter((radio) => radio.checked)[0];
										this.$set(this.filter, `${field.name}`, checked.value);
									} else if (field.type === 'select') {
										const selected = field.values.filter((select) => select.selected)[0];
