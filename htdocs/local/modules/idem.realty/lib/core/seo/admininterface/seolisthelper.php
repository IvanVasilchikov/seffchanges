 ...
 * {@inheritdoc}
 */
class SeoListHelper extends AdminListHelper
{
    ...
    protected function getData($className, $filter, $select, $sort, $raw)
    {
        ...
        foreach ($filter as $field => $searchVal) {
            ...
            elseif($field == 'LINK'){
                if (empty($sqlFilter))
                    $sqlFilter = " WHERE LINK LIKE {$searchVal}";
                else
                    $sqlFilter .= " AND LINK LIKE {$searchVal}";
            }
            else {
                if (empty($sqlFilter))
        ...
        $countResult =  $connection->query('select ID from i_seo'.$sqlFilter)->fetchAll();
