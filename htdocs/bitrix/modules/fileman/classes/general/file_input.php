 ...
 */
	...
	)
	{
		...
		foreach($values as $inputName => $fileId)
		{
			...
			{
				$arFile['FILE_NOT_FOUND'] = false;
				$arFile['INPUT_NAME'] = $inputNameTemplate."[".$inputName."]";
				$arFile['DEL_NAME'] = self::GetInputName($inputNameTemplate."[".$inputName."]", '_del');
				$arFile['DESC_NAME'] = self::GetInputName($inputNameTemplate."[".$inputName."]", '_descr');
